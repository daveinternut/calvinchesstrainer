...
| Backend | Firebase (Auth, Firestore, Analytics) — project: calvin-chess-trainer |
...
| File | Purpose |
|---|---|
| `lib/main.dart` | Entry point, wraps app in ProviderScope. Initializes Firebase via `Firebase.initializeApp()` before `runApp`. Portrait-only orientation lock via `SystemChrome.setPreferredOrientations`. Splash screen preserve/remove lifecycle via `flutter_native_splash`. |
| `lib/app.dart` | MaterialApp.router, GoRouter routes, theme. All forward navigation uses `context.push()`, all back navigation uses `context.pop()` for proper slide animations. `FirebaseAnalyticsObserver` on GoRouter auto-logs `screen_view` events for every route change. Each route has a `name` for clean analytics screen names. |
| `lib/firebase_options.dart` | Generated by FlutterFire CLI. Platform-specific Firebase config (API keys, app IDs) for Android, iOS, and web. Do not edit manually — regenerate with `flutterfire configure`. |
...
| `flutterfire_cli` (global) | Run `flutterfire configure` to regenerate `lib/firebase_options.dart` and platform config files. Installed via `dart pub global activate flutterfire_cli`. Requires Firebase CLI login (`firebase login`). Multi-account: `firebase login:add` / `firebase login:use`. |
...
- **Firebase** — project `calvin-chess-trainer` (dave@internut.education). Configured via FlutterFire CLI (`flutterfire configure`). Regenerate config with the same command if platforms or project change. Adding new Firebase dependencies requires a cold rebuild (hot restart won't pick up native SDK changes).
- **Analytics** — Firebase Analytics tracks two layers: (1) **automatic screen views** via `FirebaseAnalyticsObserver` on GoRouter — every route navigation logged with the route name; (2) **custom drill events** logged from game providers — `*_drill_started` and `*_drill_completed` with parameters (mode, subject/drill/piece, score, accuracy, bestStreak, isNewRecord, elapsedSeconds). View data in Firebase Console → Analytics → Events.
